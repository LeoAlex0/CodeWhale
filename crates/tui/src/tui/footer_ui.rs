...
pub(crate) fn render_footer(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    let toast = quit_prompt.or_else(|| {
        // Ephemeral status toasts take precedence over the version hint —
        // they are time-limited and carry actionable info. The persistent
        // version hint fills in when no ephemeral toast is active.
